...
//TODOfunciones auxiliar para probar modo RX
// Comentar cuando se tenga las 2 placas
/******************************************************************************************************/

void nrf24_CE_high_RX(void){
	LPC_GPIO0->FIOSET |= (1<<PIN_CE_NRF24_RX);
}

void nrf24_CE_low_RX(void){
	LPC_GPIO0->FIOCLR |= (1<<PIN_CE_NRF24_RX);
}


void nrf24_writeToNrf_RX(uint8_t readWrite, uint8_t reg, uint8_t *valArray, uint8_t length) {
 ...
 * 				 0 RX Mode
 * 				 */
void nrf24_init_RX(uint8_t tx_rx) {
	delay_ms(100);
	...
	//EN_AA
	*p = RF24_EN_AA_ENAA_P0;
	nrf24_writeToNrf_RX(W, RF24_EN_AA, value, sizeof(value));
	//Enable data pipe 0
	*p = RF24_EN_RXADDR_ERX_P0;
	nrf24_writeToNrf_RX(W, RF24_EN_RXADDR, value, sizeof(value));
	//RF address width setup
	*p = RF24_SETUP_AW_5BYTES;
	nrf24_writeToNrf_RX(W, RF24_SETUP_AW, value, sizeof(value));
	//RF channel freq
	*p = RF24_RF_CH_DEFAULT_VAL;
	nrf24_writeToNrf_RX(W, RF24_RF_CH, value, sizeof(value));
	//RF power mode
	*p = 0x07;
	nrf24_writeToNrf_RX(W, RF24_RF_SETUP, value, sizeof(value));
	//RX RF addr setup

	nrf24_writeToNrf_RX(W, RF24_RX_ADDR_P0, rfAddr, sizeof(rfAddr));
	if(tx_rx == W)
		nrf24_writeToNrf_RX(W, RF24_TX_ADDR, rfAddr, sizeof(rfAddr)); //Esta es sola para el transmisor
	//Payload width setup
	*p = PAYLOAD_WIDTH; //PAYLOAD_WIDTH bytes (max 32 bytes);
	nrf24_writeToNrf_RX(W, RF24_RX_PW_P0, value, sizeof(value));
	//Config register
	//bit 7 = reserv, bit 6 = MASK_RX_DR,bit 5 = MASK_TX_DS,bit 4 = MASK_MAX_RT,
	//bit 3 = EN_CRC, bit 2 = CRCO,      bit 1 =PWR_UP,     bit 0 = PRIM_RX.
	if(tx_rx == W)
		...
		*p= 0b00111111; //solo RX
	nrf24_writeToNrf_RX(W, RF24_CONFIG, value, sizeof(value)); //como transmisor
	if(tx_rx == W){
		//sets the number of retries and the retry delay like this
		*p = 0x2F; 		//0b0010_0011 "2" sets it up to 750uS delay between every retry (at least 500us at 250jbps and if payload >5bytes in 1Mbps,
						//and if payload >15byte in 2Mbps) 'F' is number of retries (1-15, now 15)
		nrf24_writeToNrf_RX(W,RF24_SETUP_RETR,value,1);
	}

	*p= RF24_STATUS_DEFAULT_VAL | RF24_MASK_MAX_RT | RF24_MASK_TX_DS | RF24_MASK_RX_DR; //limpio el status
	nrf24_writeToNrf_RX(W,RF24_STATUS,value,1);
