...
void confSpi(void){
	...
	LPC_SSP0->DR = 0; //limpio lo que tenga el registro
}
...
void RF_CSN_HIGH(){
	LPC_GPIO0->FIOSET |=(1<<PIN_SSEL0);  //high para desabilitar el modulo
}

void RF_CSN_LOW(){
	LPC_GPIO0->FIOCLR |= (1<<PIN_SSEL0); //low para habilitar el modulo
}
...
/*******************************************************************************************************/
void RF_CSN_HIGH_RX(){
	LPC_GPIO0->FIOSET |=(1<<PIN_SSEL1);  //high para desabilitar el modulo
}

void RF_CSN_LOW_RX(){
	LPC_GPIO0->FIOCLR |= (1<<PIN_SSEL1); //low para habilitar el modulo
}
